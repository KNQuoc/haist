  ...
  return (
    ...
    >
      ...
      {expanded && toolCall.result !== undefined && (
        <div className="mt-2">
          <pre className="p-2 bg-background/50 rounded text-xs overflow-auto max-h-40 whitespace-pre-wrap break-words">
      ...
      {expanded && toolCall.error && (
        <p className="mt-2 text-xs text-red-500">{toolCall.error}</p>
      )}
    </div>
  );
}
